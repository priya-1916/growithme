 ...
 const formatScheduleDate = (dateString, timeString) => {
  ...
  const handleAddWateringSchedule = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://grow-backend-2.onrender.com/api/watering/watering-schedules', {
        method: 'POST',
        headers: {
...
const handleSendEmail = async (scheduleId, plantName, wateringDate, wateringTime) => {
  try {
    ...
    const response = await fetch('https://grow-backend-2.onrender.com/api/email/send-reminder', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
